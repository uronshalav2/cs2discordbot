...
ROUND(SUM(damage)/NULLIF(COUNT(DISTINCT CONCAT(matchid,mapnumber)),0)/30,1) A
...
return _json_response({"meta": meta, "maps": maps, "players": players, "demo": except Exception as e:
return _json_response({"error": str(e)})
demo})
...
meta['team1_name'] = (raw_meta.get('team1') or {}).get('name', '') or meta['team2_name'] = (raw_meta.get('team2') or {}).get('name', '') or meta['team1_score'] = (raw_meta.get('team1') or {}).get('score', '')
...
result.sort(key=lambda x: x.get('filename_ts') or x.get('modified_at') or '', reverse=Tru
...
SUM(v1_wins) + SUM(v2_wins) AS clutch_tota
SUM(entry_wins) AS entry_wins
SUM(entry_count) AS entry_atte
ROUND(SUM(entry_wins)/NULLIF(SUM(entry_count),0)*100,1) AS entry_rate,
SUM(flash_successes) AS flash_succ
ROUND(SUM(flash_successes)/NULLIF(COUNT(DISTINCT CONCAT(matchid,'_',mapnumber
...
player_list.append({"name": p.get("name", ""), "score": 0, "duration": return _json_response({
"online": True,
...
def get_matchzy_player_stats(steamid64: str = None, player_name: str = None) -> dict | conn = get_db()
try:
None:
...
c.execute(f'''
SELECT
SUBSTRING_INDEX(GROUP_CONCAT(name ORDER BY matchid DESC), ',', 1) AS player_n
steamid64,
COUNT(DISTINCT matchid) AS matches_played
SUM(kills) AS kills,
SUM(deaths) AS deaths,
...
if not force_refresh and _MATCHID_DEMO_CACHE is not None and _MATCHID_CACHE_TIME is not N
...
print(f"[Demo Map] ✓ match {matchid} ← {name}" + (f" + {dem_entry.get('name')}" if de
...
r = requests.get(f"https://open.faceit.com/data/v4/players?nickname={nickname}", headers=
if r.status_code == 404:
raise ValueError("Player not found on FACEIT.")
r.raise_for_status()
pdata = r.json()
pid = pdata.get("player_id")
s = requests.get(f"https://open.faceit.com/data/v4/players/{pid}/stats/{FACEIT_GAME_ID}",
...
embed.add_field(name=" Players", value=f"`{player_count}/{info.max_players}`",
embed.add_field(name=" Connect", value=f"`connect {SERVER_IP}:{SERVER_PORT}`",
...
embed.set_footer(
text="Last updated",
icon_url="https://cdn.cloudflare.steamstatic.com/steamcommunity/public/images/app
)
except:
return embed, info
embed = discord.Embed(
...
self.clear_items()
if self.offset > 0:
prev_btn = Button(label="◀ Previous", style=discord.ButtonStyle.secondary, prev_btn.callback = self.previous_page
self.add_item(prev_btn)
next_btn = Button(label="Next ▶", style=discord.ButtonStyle.primary, custom_id="next"
next_btn.callback = self.next_page
self.add_item(next_btn)
refresh_btn = Button(label=" Refresh", style=discord.ButtonStyle.success, custom_id
...
return {"demos": ["Access Denied (403). URL may have expired."], "has_more": Fals
...
if aces: embed.add_field(name=" Aces (5K)", value=f"**{aces}**", inline=Tr
if clutch_wins: embed.add_field(name=" 1vX Wins", value=f"**{clutch_wins}**", inline=
if entry_wins: embed.add_field(name=" Entry Wins", value=f"**{entry_wins}**", inline=T
...
@owner_only()
async def cssban(inter: discord.Interaction, player: str, minutes: int, reason: str = "No rea
await inter.response.defer(ephemeral=True)
...
f" Map `{map}` not allowed.\nAllowed: {', '.join(MAP_WHITELIST)}", ephemeral=Tr
)
await inter.response.defer(ephemeral=True)
...
return [app_commands.Choice(name=m, value=m) for m in MAP_WHITELIST if current.lower() in
...
c.close(); conn.close()
return await inter.followup.send("\n".join(lines), ephemeral=True)
c.execute(f"SELECT name, team, kills, deaths, mapnumber FROM {MATCHZY_TABLES['players
players = c.fetchall()
lines.append(f"\n**Players ({len(players)} total):**")
for p in players:
lines.append(f"• {p['name']} | Team: `{p['team']}` | Map: {p['mapnumber']} | K/D:
c.execute(f"SELECT * FROM {MATCHZY_TABLES['maps']} WHERE matchid=%s", (match_id,))
maps = c.fetchall()
lines.append(f"\n**Maps ({len(maps)} total):**")
for m in maps:
lines.append(f"• Map {m.get('mapnumber')}: {m.get('mapname')} ({m.get('team1_scor
c.close(); conn.close()
except Exception as e:
lines.append(f"\n Error: {e}")
message = "\n".join(lines)
...
@bot.tree.command(name="debugdemos", description="Show match ID to demo mapping from .json fi
@owner_only()
...
sorted_matches = sorted(matchid_map.items(), key=lambda x: int(x[0]) if x[0].isdi
...
lines.append(f"\n Cache age: {age:.0f}s (refreshes every {_CACHE_TTL_SECOND
except Exception as e:
lines.append(f"\n Error: {e}")
message = "\n".join(lines)
if len(message) > 2000:
