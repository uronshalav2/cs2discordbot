...
return [" DEMOS_JSON_URL is not configured in environment variables."]
try:
# Fetch the JSON data
...
return [f" Network Error: Could not fetch demos. {str(e)}"]
except json.JSONDecodeError:
return [f" Parse Error: Invalid JSON response from server."]
except Exception as e:
...
embed.add_field(name="Players", value=f"{info.player_count}/{info.max_players}", inline=T
embed.add_field(
name=" Connect IP",
...
@commands.guild_only()
@commands.is_owner()
async def sync(ctx, guilds: commands.Greedy[discord.Object] = None, spec: Optional[Literal["~
...
await inter.response.defer()
lst = fetch_demos()
embed = discord.Embed(title=" await inter.followup.send(embed=embed)
Latest Demos", description="\n".join(lst), color=0x00FF00
